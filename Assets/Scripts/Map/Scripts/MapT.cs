...
namespace YandexMaps
{
    ...
    public class MapT
    {
        ...
        /// </summary>
        public static List<string> SetMarker = new List<string>();
        public static List<string> SetZone = new List<string>();
        /// <summary>
        /// Долгота.
        /// </summary>
        ...
        private static string[] ConvertArry(List<string> vectors)
        {

            string[] convert = new string[vectors.Count];

            for (int i = 0; i < vectors.Count; i++)
            {

                convert[i] = Convert(vectors[i]);
        ...
        private static string Convert(Vector3 vector)
        {
            string convertX = vector.x.ToString().Replace(',', '.');
            string convertY = vector.y.ToString().Replace(',', '.');
            string convertZ = vector.z.ToString();

            return convertX + "," + convertY + "," + convertZ;
        }
        ...
        private static string Yandex(bool layer)
        {
            var url = layer
                ...
                "&pl=" + string.Join("~", ConvertArry(SetZone))
                : "https://static-maps.yandex.ru/1.x/?ll=" +
                Convert(Longitude) + "," +
                Convert(Latitude) +
                "&size=" + Width.ToString() + "," + Height.ToString() +
